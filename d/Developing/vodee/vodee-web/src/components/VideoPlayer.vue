<template>
    <div class="video-player" :style="{ width: width, height: height }">
        <div class="video-container">
            ...
            <!-- 视频封面遮罩 -->
            <div v-if="showPoster && src && !isPlaying && currentTime === 0" class="video-poster-overlay" @click="handlePlay">
                <img :src="poster" alt="Video Poster" class="poster-image" />
            </div>
        </div>

        <!-- 控制栏 -->
        <div v-if="src && showControls" class="controls" :class="{ 'controls-hidden': !showControls }" @mouseenter="showControls = true" @mouseleave="hideControlsIfNotPlaying">
            <!-- 播放/暂停按钮 -->
            <button class="control-btn play-pause-btn" @click="togglePlay" :title="isPlaying ? '暂停' : '播放'" :disabled="!src">
            ...
            <!-- 音量控制 -->
            <button class="control-btn volume-btn" @click="toggleMute" :title="isMuted ? '取消静音' : '静音'" :disabled="!src">
                <span v-if="volume === 0 || isMuted">🔇</span>
                <span v-else-if="volume > 0.5">🔊</span>
                <span v-else>🔉</span>
            </button>

            <input v-if="src" type="range" class="volume-slider" :min="0" :max="1" step="0.01" v-model.number="volume" @input="onVolumeChange" />

            <!-- 全屏按钮 -->
            <button class="control-btn fullscreen-btn" @click="toggleFullscreen" :title="isFullscreen ? '退出全屏' : '全屏'" :disabled="!src">
...
<script setup lang="ts">
    ...
    watch(
        () => props.autoplay,
        async (newAutoplay) => {
            if (newAutoplay && videoRef.value && props.src) {
                await nextTick();
                play();
            }
        },
