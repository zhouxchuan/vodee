...
const ALLOWED_EXTENSIONS = config.allowedExtensions || ['.mp4', '.avi', '.mov', '.mkv', '.wmv', '.flv', '.webm'];
const ENABLE_ANTI_LEECH = config.enableAntiLeech !== undefined ? config.enableAntiLeech : true;
const ALLOWED_DOMAINS = config.allowedDomains || ['localhost', '127.0.0.1'];
const LEECH_TOKEN_SECRET = config.leechTokenSecret || 'your-secret-key-for-token-generation';
...
function generateLeetchToken(filePath) {
  ...
  const hash = crypto.createHmac('sha256', LEECH_TOKEN_SECRET)
                   .update(data)
                   .digest('hex');
...
function validateLeetchToken(filePath, token) {
  ...
  const expectedHash = crypto.createHmac('sha256', LEECH_TOKEN_SECRET)
                             .update(expectedData)
                             .digest('hex');
